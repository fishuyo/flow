      ...
      if(this.label) {
        ...
        this.ctx.fillText(this.label, x + width / 2, y + height / 2);
				...
				if( typeof this.value === "number")
					...
					this.ctx.fillText(parseFloat(this.value).toFixed(2), x + width / 2, y + height*3 / 4);
      }
...
Interface.Button = function() {
  Interface.extend(this, {
    ...
    changeValue : function( xOffset, yOffset, v) {
      var v = v || null;
      if(this.hasFocus || !this.requiresFocus || v !== null) {
    ...
    mouseup   : function(e) {
      if(this.mode === 'momentary') {
        this.changeValue(0,0,0);// e.x - this.x, e.y - this.y ); 
        this.isMouseOver = false;
      }
    },
    
    touchstart : function(e, hit) {
      if(hit && !this.isTouchOver) {
        console.log(e);
    ...
    touchend   : function(e) {
      console.log(e, this.touchID)
      if(e.identifier === this.touchID){
        this.isTouchOver = false;
        if(this.mode === 'momentary')
          this.changeValue(0,0,0);
      }
    },
  })
  .init( arguments[0] );
...
Interface.ButtonV = function() {
  Interface.extend(this, {
    ...
    changeValue : function( xOffset, yOffset, newvalue) {
      var newvalue = newvalue || null;
      if(this.hasFocus || !this.requiresFocus || newvalue !== null) {
        this._value = !this._value;
    ...
    setValue : function(value, doNotDraw) {
      console.log("setValue!!!  ",value, doNotDraw)
    ...
    touchmove : function(e, hit) {
      if(!this.requiresFocus && hit && !this.isTouchOver) {
        ...
        this.touchID = e.identifier;
        if(this.mode !== 'contact') {
          this.changeValue(0,0,1);// e.x - this.x, e.y - this.y );
        }else{
          this._value = 1;
          this.draw();
          var self = this;
      ...
      }else if(!hit && this.isTouchOver && this.touchID == e.identifier) {
        this.isTouchOver = false;
        if(this.mode !== 'contact') {
          this.changeValue(0,0,0);// e.x - this.x, e.y - this.y );
        }else{
    ...
    touchend   : function(e) {
      if(e.identifier === this.touchID){
        if( this.mode == 'momentary' && this.requiresFocus || ( !this.requiresFocus && this.isTouchOver) ) {
          this.changeValue(0,0,0);
        }
        this.isTouchOver = false;
      }
  ...
  Interface.extend(this, {
    ...
    onkeydown: function( e ) {
      ...
      if( typeof child !== 'undefined' && child._value == 0 ) {
        child.changeValue(0,0,1)
      }
    },
    //Math.pow(2,(startnote + 12*octave - 49)/12)*261.626
    //onvaluechange : function() { this.values = [this.frequency,this.value] },
    onvaluechange : function() { 
    //  this.values = [this.startnote + 12*this.octave + 7,1-this.value] 
      //charlie's old code
      if (this.sendFrequencies) {
        this.values = [1-this.value, Math.pow(2,(this.startnote + 12*this.octave - 49)/12)*261.626];
      }
      this.additionalonvaluechange();
    },    
    onboundschange: function() { if( this._initialized) this.placeKeys() },
    
    
    additionalonvaluechange: function() {
    ...
    placeKeys: function() {
      ...
      for (var i = 0; i < notes - 1; i++ ) {
        ...
        var _key = new Interface.ButtonV({ 
          ...
          additionalonvaluechange: this.additionalonvaluechange,
          'startnote':startnote,
          'octave':octave,
          ...
          requiresFocus : false,
          mode:'momentary',
          sendFrequencies : this.sendFrequencies,
          target:this.target, key: this.key +'/'+ String((startnote-1) + 12*(octave-this.startoctave)),//String((startnote-1) + 12*(octave+2)),
        });
      ...
      if (startnote == 2 || startnote == 4 || startnote == 7 || startnote == 9 || startnote == 11)
        ...
        var pkeys = new Interface.ButtonV({ 
              ...
              mode:'momentary',
           //   sendFrequencies : this.sendFrequencies,
              target:this.target, key: this.key +'/'+ String((startnote-1) + 12*(octave-this.startoctave)),//String((startnote-1) + 12*(octave+2)),  //c3 = 60 for midi!
            });
      else
        var pkeys = new Interface.ButtonV({ 
  ...
  Interface.extend(this, {
    ...
    makeChildren : function() {
      if (this.numChildren == 1) {
        ...
        this.children.push({ id:0, x:x, y:y, vx:0, vy:0, collideFlag:false, isActive:false, lastPosition:null, });
        this.values.push({ x:.5, y:.5 });
      } else {
