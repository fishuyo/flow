  ...
  joy.leftX.map(2 * _ - 1).map(dd(_)).map(_ * v) >> app.sinks("mx")
  joy.leftY.map(2 * _ - 1).map(dd(_)).map(_ * -v) >> app.sinks("mz")
  joy.rightX.map(2 * _ - 1).map(dd(_)).map(_ * -w) >> app.sinks("ty")
  joy.rightY.map(2 * _ - 1).map(dd(_)).map(_ * -w) >> app.sinks("tx")
