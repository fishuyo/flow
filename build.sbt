...
lazy val jsSettings = Seq(
  ...
  webpackConfigFile := Some((baseDirectory.in(flow)).value / "webpack" / "custom.webpack.config.js"),
  fastOptJS / webpackBundlingMode := BundlingMode.LibraryAndApplication(), 
)

lazy val flow:Project = project.in(file("."))
  .aggregate(server)
...
lazy val server = project.in(file("modules/server"))
  ...
  .dependsOn(flow_service, launcher_service)
...
lazy val launcher_service = project.in(file("modules/services/launcher/service"))
  .settings(
    scalaJSProjects := Seq(launcher_client),
    Assets / pipelineStages := Seq(scalaJSPipeline),
    Compile / compile := ((Compile / compile) dependsOn scalaJSPipeline).value,
  ...
  .enablePlugins(WebScalaJSBundlerPlugin)
  .dependsOn(launcher_shared.jvm, util)


lazy val launcher_client = project.in(file("modules/services/launcher/client"))
  .settings(
    scalaJSUseMainModuleInitializer := true,
    libraryDependencies ++= Seq(
      // "org.scala-js" %%% "scalajs-dom" % "2.1.0",
  ...
  .enablePlugins(ScalaJSPlugin, ScalablyTypedConverterPlugin, ScalaJSBundlerPlugin)
  .dependsOn(launcher_shared.js)


lazy val launcher_shared = crossProject(JSPlatform, JVMPlatform)
  .crossType(CrossType.Pure)
  .in(file("modules/services/launcher/shared"))
  .settings(
    libraryDependencies ++= Seq(
      "com.lihaoyi" %%% "upickle" % "3.1.4"
    )
